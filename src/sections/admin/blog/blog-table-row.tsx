...
import { EBannerStatus, IBannerItem } from 'src/types/banner';
...
export default function BlogTableRow({
  ...
  const { imageSrc, alias, title, link, description, status, startAt, endAt } = row;
      ...
      <TableRow hover selected={selected}>
        ...
        <TableCell>
          <Label
            variant="soft"
            color={
              (status === EBannerStatus.ACTIVE && 'success') ||
              (status === EBannerStatus.INACTIVE && 'warning') ||
              'default'
            }
          >
            {status ? 'Вкл.' : 'Викл.'}
