...
import { EArticleStatus } from 'src/types/article';
...
export default function BlogDrawerForm({ closeDrawer, coupon }: Props) {
	...
	const FormSchema = Yup.object().shape({
		...
		status: Yup.string().oneOf([EArticleStatus.ACTIVE, EArticleStatus.INACTIVE]).required('Статус є обов`язковим'),
	});

	const defaultValues = { images: [], alias: '', title: '', description: '', status: EArticleStatus.INACTIVE, startAt: '', endAt: '' };
	...
	return (
		<FormProvider methods={methods} onSubmit={handleSubmit(onSubmit)}>
			<DrawerFormHeader title='Створення статті' onClose={closeDrawer} />
				...
				<Scrollbar>
					...
					<DrawerTitleDivider number={3} title='Стан'>
						<RHFSelect native name='status' label='Статус'>
							<option value={EArticleStatus.ACTIVE}>
								Активний
							</option>
							<option value={EArticleStatus.INACTIVE}>
								Виключений
							</option>
						</RHFSelect>
					</DrawerTitleDivider>
