...
import { EProductStatus } from 'src/types/product';
...
export default function ProductsDrawerForm({ closeDrawer, coupon }: Props) {
	const FormSchema = Yup.object().shape({
		alias: Yup.string().required('Алиас є обов`язковим'),
		categoryId: Yup.number().required('Категорія є обов`язковим'),
		title: Yup.string().required('Назва є обов`язковим'),
		code: Yup.string().required('Код є обов`язковим'),
		description: Yup.string().required('Опис'),
		images: Yup.array().min(1, 'Images is required'),
		status: Yup.string().oneOf([EProductStatus.ACTIVE, EProductStatus.INACTIVE]).required('Статус є обов`язковим'),
	});

	const defaultValues = { categoryId: 0, images: [], code: '', alias: '', title: '', description: '', status: EProductStatus.INACTIVE };
	...
	return (
		<FormProvider methods={methods} onSubmit={handleSubmit(onSubmit)}>
			<DrawerFormHeader title='Створення продукту' onClose={closeDrawer} />
			...
			<Box sx={{ height: 'calc(100vh - 70px - 81px)' }}>
				<Scrollbar>
					<DrawerTitleDivider number={1} title='Головне'>
						...
						<RHFTextField name='code' label='Код' type='text' />
					...
					<DrawerTitleDivider number={3} title='Стан'>
						<RHFSelect native name='status' label='Статус'>
							<option value={EProductStatus.ACTIVE}>
								Активний
							</option>
							<option value={EProductStatus.INACTIVE}>
								Виключений
							</option>
						</RHFSelect>
					</DrawerTitleDivider>
