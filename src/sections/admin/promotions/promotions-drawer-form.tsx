...
import { EPromotionStatus } from 'src/types/promotion';
...
export default function PromotionsDrawerForm({ closeDrawer, coupon }: Props) {
	const FormSchema = Yup.object().shape({
		...
		status: Yup.string().oneOf([EPromotionStatus.ACTIVE, EPromotionStatus.INACTIVE]).required('Статус є обов`язковим'),
		startAt: Yup.string().required('Дата початку є обов`язковим'),
		endAt: Yup.string().required('Дата закінчення є обов`язковим'),
	});

	const defaultValues = { images: [], alias: '', title: '', description: '', status: EPromotionStatus.INACTIVE, startAt: '', endAt: '' };
	...
	return (
		<FormProvider methods={methods} onSubmit={handleSubmit(onSubmit)}>
			<DrawerFormHeader title='Створення акції' onClose={closeDrawer} />
				...
				<Scrollbar>
					...
					<DrawerTitleDivider number={3} title='Стан'>
						<RHFSelect native name='status' label='Статус'>
							<option value={EPromotionStatus.ACTIVE}>
								Активний
							</option>
							<option value={EPromotionStatus.INACTIVE}>
								Виключений
							</option>
						</RHFSelect>
					</DrawerTitleDivider>
